...
class Sections extends Resource
{
    ...
    {
        if ($id !="") {
			$query = "SELECT * FROM sections where pid=".$id or die("Error in the consult.." . mysqli_error($link));
			//echo $query;
		} else {
			...
			while($row = mysqli_fetch_assoc($result)) {
				//print_r($row);
				$sections["sections"][$row['rank']] = $row;
				$query = "SELECT * FROM subsections where sid=".$row['id'] or die("Error in the consult.." . mysqli_error($link));
				//echo $query;
				$sresult = $link->query($query);
				//print_r($result);
				while($srow = mysqli_fetch_assoc($sresult)) {
					//print_r($srow);
					$sections["sections"][$row['rank']]['subsections'][$srow['rank']]['id'] = $srow['id'];
